...
class RunDayTable(tables.Table):
    ...
    def render_weekday(self, value, record):
        ...
        return mark_safe('<a href="/por/editday?id=' + str(record.id) + '">' + str(value) + '</a>')
...
class RunHeaderTable(tables.Table):
    ...
    def render_id(self, value, record):
        ...
        return mark_safe('<a href="/por/editrun?run_id=' + str(record.id) + '">' + '<i class="fa fa-edit fa-lg"></i> ' + '</a>')

    def render_run_name(self, value, record):
        vote_count = 0
        data = RunHeaders.objects.filter(id=record.id)

        return mark_safe('<a href="/por/home/?run_id=' + str(record.id) + '">' + str(value) + '</a>')
...
class RunDetailsTable(tables.Table):
    ...
    def render_id(self, value, record):
        ...
        return mark_safe(
            '<a href="/por/editdetails?id=' + str(record.id) + '">' + '<i class="fa fa-edit fa-lg"></i> ' + '</a>')

    def render_station_number(self, value, record):
        data = RunDetails.objects.filter(id=record.id).select_related("station")

        return mark_safe('<a href="/por/editdetails?id=' + str(record.id) + '">' + str(value) + '</a>')
    ...
    def render_run_status(self, value, record):
        ...
        if data[0].run_status == 'Closed':
            #status = '<span class="stamp stamp-declined">Closed</span>'
            status = '<label class ="switch"> <input type = "checkbox" disabled> <span class ="slider round"> </span> </label>'
        else:
            #status = '<span class="stamp stamp-approved">Open</span>'
...
class StationTable(tables.Table):
    ...
    def render_station_number(self, value, record):
        ...
        return mark_safe('<a href="/por/editstation?id=' + str(record.id) + '">' + str(value) + '</a>')
